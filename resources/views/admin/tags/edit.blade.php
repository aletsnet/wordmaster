...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold">{{ __('admin.edit_tag') }}</h2>
</div>
<form action="{{ route('admin.tags.update', $tag) }}" method="POST" class="bg-white rounded-lg shadow p-6">
    ...
    <div class="mb-4">
        ...
        <input type="text" name="name" value="{{ old('name', $tag->name) }}" class="w-full border rounded px-3 py-2" required>
    </div>
    <div class="mb-4">
    ...
    <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">{{ __('admin.update_tag') }}</button>
</form>
@endsection
