...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold">{{ __('admin.new_post') }}</h2>
</div>
<form action="{{ route('admin.posts.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
    @csrf
    <div class="mb-4">
        <label class="block text-gray-700 mb-2">{{ __('admin.title') }}</label>
        <input type="text" name="title" value="{{ old('title') }}" class="w-full border rounded px-3 py-2 @error('title') border-red-500 @enderror" required>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
            ...
            <input type="text" name="template" value="{{ old('template') }}" class="w-full border rounded px-3 py-2" placeholder="{{ __('admin.template_placeholder') }}">
        </div>
    </div>
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <div class="grid grid-cols-3 gap-2">
            @foreach($tags as $tag)
            <label class="flex items-center">
                <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="mr-2"> {{ $tag->name }}
            </label>
            @endforeach
        </div>
    </div>
    <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">{{ __('admin.create_post') }}</button>
</form>
